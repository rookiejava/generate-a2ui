...
export default async function handler(req: any, res: any) { try { const version = req.body?.version ?? 'v0.10'; const messages = parseMessages(String(req.body?.source ?? '[]')); const validation = await validateMessages(version, messages); res.status(200).json({ validation, preview: createPreviewDocument(version, messages as any[]) }); } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' }); } }
